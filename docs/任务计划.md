...
| TASK-011 | 模块清单与代码巡检 | ☑ 进行中 | 2026-01-07 | - |
| TASK-012 | 并发一键开启/关闭设备 | ☑ 进行中 | 2026-01-07 | - |
...
- DefaultSerialPortAdapter 封装 System.IO.Ports.SerialPort，提供默认超时与编码，Open/Close/Dispose 幺端性可进一步校验。
...
- 适配器：保证 Open/Close/Dispose 幺端，DataReceived 订阅对称，编码/超时参数校验与默认值兜底。
...
### 执行记录
- 2026-01-07 启动巡检任务，已完成模块职责确认与巡检计划准备。
...
### 任务描述
为多设备增加并发方式的一键开启/关闭功能，在保证线程安全与设备隔离的前提下提升执行速度，并提供结果反馈。

### 任务分析
- 范围：多设备一键开启/关闭（避免串行阻塞）。
- 并发：控制并发度（如 2~3），避免资源争用；失败收敛汇总。
- 校验：仅对已锁定且已连接的设备执行；未锁定/未连接直接提示。
- 超时/重试：单设备调用设置超时、可选退避重试 1~2 次。
- 反馈：UI 禁用按钮、显示进度/成功失败计数，完成后刷新状态。

### 任务要求
- Task 使用中文；更新任务计划。
- 修改代码直接修改文件，不使用终端传输代码。
- 保持中文注释一致性。
- 验证构建成功；必要时补充/更新测试。

### 步骤清单
- [x] 设计并发策略与并发度（SemaphoreSlim 等），定义超时/重试/校验规则
- [x] 在协调器/服务层实现一键开启/关闭并发逻辑（含校验、超时、重试、结果汇总）
- [x] UI 接入：按钮禁用/启用，显示执行结果，刷新状态
- [x] 并发度配置化（AppConfig.PowerConcurrency + JSON 序列化/反序列化，调用读取配置）
- [ ] 补充或更新测试（并发路径、失败汇总）
- [x] 构建/测试验证
- [x] 更新任务计划状态与执行记录

### 执行记录
- 2026-01-07 完成并发一键开/关实现：Coordinator 增加 TurnOnAllAsync/TurnOffAllAsync（并发度可控，锁定/连接校验、结果汇总）；MainForm 新增一键开/关按钮并接入；Release 构建通过。
- 2026-01-07 并发数配置化：AppConfig 增加 PowerConcurrency，ConfigRepository 读写 powerConcurrency 字段，Coordinator 默认读取配置（参数可覆盖）；构建通过。
