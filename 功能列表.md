...
### 3. 配置管理
- 持久化文件（`Config` 目录）：
  - `serialport.config`：最后使用的串口
  - `portlock.config`：串口锁定状态
  - `devicename.config`：设备名称（默认 “FCC1电源”）
- 默认与重试参数：`appsettings.json` 绑定 `AppConfig`（串口默认值 + `RetryPolicy`：ConnectRetries/SendRetries/BaseDelayMs）
- 设置界面：串口下拉、锁定/解锁、监视器开关；确认后保存
