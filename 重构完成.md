...
### 2. 依赖管理
```
MainForm ? IDeviceController ? ISerialPortService
  ↓       ↓           ↓
...
| 指标 | 重构前 | 重构后 | 改善 |
|------|-------|--------|------|
| **代码行数** (MainForm) | ~450行 | ~200行 | **-56%** |
| **方法数量** (MainForm) | 18个 | 8个 | **-56%** |
| **重复代码** | ~30% | 0% | **-100%** |
| **类的职责** | 7个 | 1个 | **-86%** |
| **可维护性** | 中等 | 优秀 | **+100%** |
...
### 稳定性增强 (2025-12-22)
- **设置窗口复用**：改为无模式窗口，避免重复创建/销毁开销，支持主窗体并发操作。
- **事件解绑**：主窗体关闭或设置窗口关闭时，严格解绑事件，防止内存泄漏。
- **异常恢复**：连接操作增加 try/catch 保护，失败时自动恢复按钮状态，避免 UI 死锁。
- **配置校验**：引入 `TryUpdateConnectionConfig`，在保存前统一校验模型有效性。
