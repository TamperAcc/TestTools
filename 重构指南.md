...
## 扩展策略
- 多协议：实现 `IProtocolParserFactory`/`IProtocolParser`，在 DI 覆盖默认解析器。
- 多设备：实现 `IDeviceControllerFactory`/`IDeviceController`，在 DI 注册不同控制器。
- 多适配器：实现 `ISerialPortAdapterFactory`/`ISerialPortAdapter`，可接入虚拟/自定义串口。

## 测试约定
- 优先依赖接口 + 工厂，单测可注入 fake/mock。
- 协调器：验证事件转发、配置保存和异常路径。
- 串口服务：验证重试策略、连接/发送的异常分支（用假适配器）。
- 控制器：验证帧解析驱动状态变更。
